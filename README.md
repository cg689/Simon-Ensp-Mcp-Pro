...
**🤖 AI 用户（最省事的路）**：把整个仓库下载/克隆到你电脑，然后在你的 AI 助手（WorkBuddy / Claude Code / Cursor 等）里说：

> 「我用了一个项目在 `C:\...\Simon-Ensp-Mcp-Pro`，帮我用它搭一个拓扑：两台 AR2220 + 一台 S5700 + 两台 PC，R1、R2 下联交换机，PC 接交换机。」

AI 会自动读根目录的 **`AGENTS.md`**（写给 AI 看的执行手册），替你完成生成、自检、交付——你只需要把产出文件拖进 eNSP 确认。
