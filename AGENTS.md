...
你读到本文件有两种方式：①用户把本仓库设为工作区（本文件自动生效，`REPO_ROOT` = 工作区根目录）；②用户在对话里给出了仓库路径。如果用户已经安装了 `ensp-topo-generate` 技能，优先用技能（自含模板与补丁，不依赖本仓库位置），本手册作为方法论的完整参考。
...
更完整的模板：`功能1-生成拓扑/`（模板脚本.py / 一键体验.py），需要细节时读 `功能1-生成拓扑/使用说明.md`。
